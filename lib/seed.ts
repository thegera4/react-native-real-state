...
const propertyTypes: string[] = ["Casa", "Cuarto", "Departamento", "Local",]

const facilities: string[] = ["Lavanderia", "Pet-friendly", "Wifi", "Patio", "Cochera", "Escritorio"]
...
function getRandomSubset<T>(array: T[], minItems: number, maxItems: number): T[] {
  ...
  const subsetSize: number = Math.floor(Math.random() * (maxItems - minItems + 1)) + minItems
...
async function seed() {
  try {
    ...
    for (const key in COLLECTIONS) {
      const collectionId: string | undefined = COLLECTIONS[key as keyof typeof COLLECTIONS]
    ...
    for (let i = 1; i <= 20; i++) {
      ...
      const selectedFacilities: string[] = facilities.sort(() => 0.5 - Math.random())
                                            .slice(0, Math.floor(Math.random() * facilities.length) + 1)

      const image: string = propertiesImages.length - 1 >= i ? propertiesImages[i]
